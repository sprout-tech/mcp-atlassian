...
| `attachments` | `string` | No | (Optional) Attachments to upload. Prefer a JSON array string. Each item is either a workspace-relative file path string (confined to the MCP server CWD; absolute paths outside the workspace are rejected), or an object with 'filename', base64-encoded 'base64' content, and optional 'mime_type' for chat-paste / remote uploads that never touch the filesystem. Example path list: ["screenshots/a.png"] or comma-separated 'screenshots/a.png,docs/b.pdf'. Example inline (from chat ImageContent.data): [`{"filename":"shot.png","mime_type":"image/png","base64":"..."}`]. At most 10 items; each inline payload capped at 50 MiB. |
...
**Chat / remote image upload:** paste or attach an image in chat, then pass MCP `ImageContent.data` as `attachments` inline objects (`filename`, `base64`, optional `mime_type`). Content is uploaded in memory — do not write chat images to `/tmp` or other paths outside the server workspace. Workspace path strings are still supported and remain confined to the MCP server CWD.
</Tip>
