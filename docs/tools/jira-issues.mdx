...
| `attachments` | `string` | No | USE THIS TO ATTACH FILES OR SCREENSHOTS TO AN EXISTING ISSUE (e.g. 'add this image to PK-123'). No separate upload tool. TWO MODES: (1) Same filesystem as the MCP server — write under the MCP CWD and pass a relative path, e.g. ["uploads/screenshot.png"]. Absolute paths outside the MCP CWD are always rejected. (2) Separate sandbox (Claude Desktop / remote agent that cannot write into the MCP install dir) — you MUST use inline base64 from ImageContent.data, e.g. [`{"filename":"screenshot.png","mime_type":"image/png","base64":"<ImageContent.data>"}`]; do not try paths to your private Desktop/uploads folders. Inline base64 limited to 5 MiB; max 10 items. |
