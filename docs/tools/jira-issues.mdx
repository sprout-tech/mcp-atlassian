...
| `attachments` | `string` | No | USE THIS TO ATTACH FILES OR SCREENSHOTS TO AN EXISTING ISSUE (e.g. user says 'add this image to PK-123'). There is no separate jira_upload_attachment tool. PREFERRED for screenshots / anything larger than a few KB: save the file under the MCP server workspace (e.g. uploads/screenshot.png) and pass a relative path JSON array like ["uploads/screenshot.png"]. Do NOT base64-encode large images into this argument (inline base64 soft-capped at 65536 bytes) — LLMs cannot reliably pass 100k+ character tool args. Tiny files only may use inline objects: [`{"filename":"tiny.png","mime_type":"image/png","base64":"<short ImageContent.data>"}`]. Absolute paths outside the MCP server CWD are rejected. Max 10 items. |
...
```json
{"issue_key": "PROJ-123", "attachments": "[{\"filename\":\"tiny.png\",\"mime_type\":\"image/png\",\"base64\":\"iVBORw0KGgo=\"}]", "return_fields": "key,summary"}
```
...
<Tip>
**Attach a screenshot / chat image:** use this tool (there is no `jira_upload_attachment`). Preferred: save the file under the MCP server workspace (e.g. `uploads/screenshot.png`) and pass a relative path — `attachments: ["uploads/screenshot.png"]`. Do **not** base64-encode large images into the tool argument (inline base64 is soft-capped ~64 KiB); LLMs cannot reliably pass 100k+ character args.

Tiny files only may use inline `{filename, base64, mime_type}` objects.
...
Absolute paths outside the MCP server CWD are rejected (`validate_safe_path`).
</Tip>
